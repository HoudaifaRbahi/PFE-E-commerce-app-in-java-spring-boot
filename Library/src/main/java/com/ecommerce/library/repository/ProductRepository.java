...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(value = "select p.product_id, p.name, p.description,p.quantity_ordered, p.current_quantity, p.cost_price, p.category_id, p.sale_price, p.image, p.is_activated, p.is_deleted from products p where p.is_deleted = false and p.is_activated = true limit 4", nativeQuery = true)
    List<Product> listViewProduct();


    @Query(value = "select p from Product p inner join Category c on c.id = ?1 and p.category.id = ?1 where p.is_activated = true and p.is_deleted = false and p.category.activated=true")
    List<Product> getProductByCategoryId(Long id);


    @Query("select p from Product p where p.name like %?1% or p.description like %?1%")
    List<Product> searchProducts(String keyword);
    @Query("SELECT p FROM Product p WHERE p.currentQuantity <= 10 ORDER BY p.currentQuantity ASC LIMIT 5")
    List<Product> nearingCompletion();
    @Query(value="SELECT * FROM products ORDER BY quantity_ordered DESC LIMIT 5", nativeQuery = true)
    List<Product> bestSeller();

    @Query(value="SELECT * FROM products ORDER BY quantity_ordered ASC LIMIT 5",nativeQuery = true)
    List<Product> LeastSeller();
